...
class Backend
{
        ...
        if ($FOLDER != '') {
            ...
            if ($listing = \OC\Files\Filesystem::opendir($FOLDER)) {
                ...
                while (($file = readdir($listing)) !== false) {
                    ...
                    if (!$this->endsWith($tmpfile, ".md") && !$this->endsWith($tmpfile, ".html")) continue;
            ...
            if ($results)
                foreach ($results as $result)
                    if ($result['deleted'] == 0) {
                        $tmpfile = $result['name'] . ".md";
                        if ($result['grouping'] != '')
                            $tmpfile = '[' . $result['grouping'] . '] ' . $result['name'] . '.md';
                        $filefound = false;
    ...
    {
        $in_name = str_replace('.md', '', $in_name);
        ...
        if (!$fileindb) {
            if ($FOLDER != '') {
                $tmpfile = $FOLDER . "/" . $name . ".md";
                if ($group != '')
                    $tmpfile = $FOLDER . "/[" . $group . "] " . $name . ".md";
    ...
    {
        $name = str_replace('.md', '', $name);
        $in_newname = str_replace('.md', '', $in_newname);
